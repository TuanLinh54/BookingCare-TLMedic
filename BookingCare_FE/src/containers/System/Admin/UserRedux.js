    ...
    checkValidateInput = () => {
        ...
        for (let i = 0; i < arrCheck.length; i++) {
            if (!this.state[arrCheck[i]]) {
                isValid = false;
                alert('Thiếu thông tin bắt buộc: ' + arrCheck[i]);
                break;
            }

            if (arrCheck[i] === 'email' && !emailRegex.test(this.state.email)) {
                isValid = false;
                alert('Email không hợp lệ');
                break;
            }

            if (arrCheck[i] === 'phoneNumber' && !phoneRegex.test(this.state.phoneNumber)) {
                isValid = false;
